...
func (s *Suite) SetupSuite() {
	...
	r := s.shellSuite.Runner("../deployments-k8s/examples/multiforwarder")
	s.T().Cleanup(func() {
...
func (s *Suite) TestKernel2Kernel_Vfio2Noop() {
	r := s.shellSuite.Runner("../deployments-k8s/examples/use-cases/Kernel2Kernel&Vfio2Noop")
	s.T().Cleanup(func() {
...
func (s *Suite) TestKernel2Vxlan2Kernel_Vfio2Noop() {
	r := s.shellSuite.Runner("../deployments-k8s/examples/use-cases/Kernel2Vxlan2Kernel&Vfio2Noop")
	s.T().Cleanup(func() {
		r.Run(`NSE_VFIO=$(kubectl get pods -l app=nse-vfio -n ${NAMESPACE} --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
		r.Run(`kubectl -n ${NAMESPACE} exec ${NSE_VFIO} --container ponger -- /bin/bash -c '\` + "\n" + `  sleep 10 && kill $(pgrep "pingpong") 1>/dev/null 2>&1 &                    \` + "\n" + `'`)
		r.Run(`kubectl delete ns ${NAMESPACE}`)
...
func (s *Suite) TestSriovKernel2Noop() {
	r := s.shellSuite.Runner("../deployments-k8s/examples/use-cases/SriovKernel2Noop")
	s.T().Cleanup(func() {
		r.Run(`kubectl delete ns ${NAMESPACE}`)
	})
	r.Run(`NAMESPACE=($(kubectl create -f ../namespace.yaml)[0])` + "\n" + `NAMESPACE=${NAMESPACE:10}`)
...
func TestVfio2Noop(s *Suite) {
	r := s.shellSuite.Runner("../deployments-k8s/examples/use-cases/Vfio2Noop")
	s.T().Cleanup(func() {
...
func (s *Suite) TestVfio2Noop_loop() {
	for i := 0; i < 20; i++ {
		s.Run("", func() {
			TestVfio2Noop(s)
		})
