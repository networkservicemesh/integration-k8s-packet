...
func  TestVfio2Noop_less_echoes_timeout(s *Suite) {
	...
	r.Run(`kubectl -n ${NAMESPACE} wait --for=condition=ready --timeout=1m pod -l app=nse-vfio`)
	r.Run(`NSC=$(kubectl -n ${NAMESPACE} get pods -l app=nsc-vfio --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	r.Run(`function dpdk_ping() {` + "\n" + `  echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^ starting dpdk_ping"` + "\n" + `  err_file="$(mktemp)"` + "\n" + `  trap 'rm -f "${err_file}"' RETURN` + "\n" + `` + "\n" + `  out="$(timeout -v --kill-after=10s 30s kubectl -n ${NAMESPACE} exec ${NSC} --container pinger -- /bin/bash -c '\` + "\n" + `    /root/dpdk-pingpong/build/app/pingpong                                       \` + "\n" + `      --no-huge                                                                  \` + "\n" + `      --                                                                         \` + "\n" + `      -n 500                                                                     \` + "\n" + `      -c                                                                         \` + "\n" + `      -C 0a:11:22:33:44:55                                                       \` + "\n" + `      -S 0a:55:44:33:22:11                                                       \` + "\n" + `  ' 2>"${err_file}")"` + "\n" + `  ` + "\n" + `  if [[ "$?" != 0 ]]; then` + "\n" + `    cat "${err_file}" 1>&2` + "\n" + `    echo "${out}" 1>&2` + "\n" + `    echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^ kubectl pingpong failed, return 1"` + "\n" + `    return 1` + "\n" + `  fi` + "\n" + `` + "\n" + `  if ! pong_packets="$(echo "${out}" | grep "rx .* pong packets" | sed -E 's/rx ([0-9]*) pong packets/\1/g')"; then` + "\n" + `    cat "${err_file}" 1>&2` + "\n" + `    echo "${out}" 1>&2` + "\n" + `    echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^ creating pong_packets failed, return 1"` + "\n" + `    return 1` + "\n" + `  fi` + "\n" + `` + "\n" + `  if [[ "${pong_packets}" == 0 ]]; then` + "\n" + `    cat "${err_file}" 1>&2` + "\n" + `    echo "${out}" 1>&2` + "\n" + `    echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^ checking pong_packets failed, return 1"` + "\n" + `    return 1` + "\n" + `  fi` + "\n" + `  ` + "\n" + `  echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^ dpdk_ping success!"` + "\n" + `` + "\n" + `  echo "${out}"` + "\n" + `  return 0` + "\n" + `}`)
	r.Run(`dpdk_ping`)
}

func (s *Suite) TestVfio2Noop_2_less_echoes_timeout_loop() {
	for i := 0; i < 6; i++ {
		s.Run("", func() { TestVfio2Noop_less_echoes_timeout(s) })
	}
}
...
func (s *Suite) TestVfio2Noop_3_vanilla_loop() {
	for i := 0; i < 6; i++ {
